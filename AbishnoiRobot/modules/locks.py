...
import AbishnoiRobot.modules.sql.locks_sql as sql
from AbishnoiRobot import dispatcher, DRAGONS, LOGGER
                    ...
                    if "ARABIC" in check:
                        try:
                            ...
                            if excp.message == "Mᴇssᴀɢᴇ ᴛᴏ ᴅᴇʟᴇᴛᴇ ɴᴏᴛ ғᴏᴜɴᴅ":
                                ...
                                LOGGER.exception("ERROR ɪɴ ʟᴏᴄᴋᴀʙʟᴇs")
                        break
                if message.text:
...
def build_lock_message(chat_id):
    ...
    permslist = []
    if locks:
        res += "*" + "Tʜᴇsᴇ ᴀʀᴇ ᴛʜᴇ ᴄᴜʀʀᴇɴᴛ ʟᴏᴄᴋs ɪɴ ᴛʜɪs Cʜᴀᴛ:" + "*"
        if locks:
...
def list_locks(update, context):
    ...
    if conn:
        res = res.replace("Lᴏᴄᴋs ɪɴ", "*{}*".format(chat_name))
